...
function getBotReply(text) {
  ...
  if (t.includes('приём') || t.includes('прием') || t.includes('запись') || t.includes('врач'))
    return 'У вас запланировано 2 приёма:\n• 5 июня, 14:30 — Педиатр Смирнов А.В.\n• 9 июня, 10:00 — Логопед-Дефектолог Иванова Е.С.\n\nПеред визитом возьмите ИИН ребёнка и последние анализы.';
  ...
  if (t.includes('игр') || t.includes('упражн'))
    return 'Рекомендую начать с «Артикуляционной гимнастики» — она подходит детям 4–7 лет. Для бытовых привычек попробуйте «Полезные пары», а для первых речевых звуков — «Звукоподражание».';
  if (t.includes('температур') || t.includes('жар'))
    return 'Если температура высокая, ребёнок вялый, тяжело дышит или есть судороги — звоните 103/112. Если состояние стабильное, запишите температуру, время, лекарства и свяжитесь с педиатром.';
  if (t.includes('лекар') || t.includes('напомин'))
    return 'В медкарте есть блок «Лекарства и напоминания». Добавьте время, дозировку и короткую заметку, чтобы родителям было проще соблюдать назначения врача.';
  if (t.includes('иин'))
